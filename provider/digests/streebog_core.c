...
struct gs_streebog_ctx_st
{
    ...
    uint64_t buffer[ BlockLengthInUInt64 ];
...
static inline
void doPadding( uint64_t buffer[], size_t bufferSize )
{
...
static inline
void doAdd( uint64_t r[], const uint64_t x[], const uint64_t y[] )
{
...
static inline
void doXor( uint64_t r[], const uint64_t x[], const uint64_t y[] )
{
...
static inline
void doLPS( uint64_t r[], const uint64_t x[], const uint64_t y[] )
{
...
void doStage3( GsStreebogCtx* ctx )
{
    ...
    memcpy( ctx->buffer, buffer, sizeof( buffer ) );
    memset( buffer, 0x00, sizeof( buffer ) );

    buffer[ 0 ] = ctx->bufferSize << 3;

    doPadding( ctx->buffer, ctx->bufferSize );
    ...
    doAdd( ctx->sigma, ctx->sigma, ctx->buffer );
...
void GsStreebogCtxUpdate( GsStreebogCtx* ctx, const uint8_t* data, size_t dataSize )
{
    size_t chunkSize;
    ...
    while( dataSize )
    {
        ...
        {
            uint64_t v = 0;
            memcpy( &v, &data[ i * sizeof( uint64_t ) ], sizeof( uint64_t ) );
            ctx->buffer[ i ] = v;//bswap_64( v );
        }
...
void GsStreebogCtxFinal( GsStreebogCtx* ctx, uint8_t* digest )
{
    ...
    if( ctx->digestSize == Streebog256LengthInBytes )
    {
        memcpy( digest, &ctx->digest[ BlockLengthInUInt64 / 2 ],
                ctx->digestSize );
    }
    else
    {
