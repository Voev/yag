...
static int StreebogFinal(void* ctx, unsigned char* out, size_t* outl, size_t outsz);
...
{
    static const OSSL_PARAM table[] = {OSSL_PARAM_size_t(OSSL_DIGEST_PARAM_BLOCK_SIZE, NULL),
...
int StreebogGetParams(OSSL_PARAM params[], const size_t digestSize)
{
    ...
    if (p != NULL && !OSSL_PARAM_set_size_t(p, BlockLengthInBytes))
    {
        return 0;
    }
    p = OSSL_PARAM_locate(params, OSSL_DIGEST_PARAM_SIZE);
    if (p != NULL && !OSSL_PARAM_set_size_t(p, digestSize))
    {
        return 0;
    }
