...
void* GsKeyMgmtNew_(void* provData, int algorithm)
{
    ...
    return GsAsymmKeyNewInit(provCtx, algorithm);
}
...
int GsKeyMgmtGetParams(void* keyData, OSSL_PARAM params[])
{
    ...
    bnCtx = BN_CTX_new_ex(GsProvCtxGet0LibCtx(provCtx));
    if (!bnCtx)
    {
        return 0;
    }
    ...
    {
        p->return_size =
            EC_POINT_point2oct(GsAsymmKeyGet0Group(key), GsAsymmKeyGet0PublicKey(key),
                               POINT_CONVERSION_UNCOMPRESSED, p->data, p->return_size, bnCtx);
        if (p->return_size == 0)
            goto end;
    }
    ...
    if (p)
    {
        ...
        size_t publicKeyLen =
            EC_POINT_point2buf(group, publicKey, POINT_CONVERSION_COMPRESSED, &publicKeyBuf, bnCtx);
...
int GsKeyMgmtSetParams(void* keyData, const OSSL_PARAM params[])
{
    ...
    if (p)
    {
        if (p->data_type != OSSL_PARAM_OCTET_STRING ||
            !GsAsymmKeyDecodePublicKey(key, params->data, params->data_size))
        {
            return 0;
        }
    }
...
int GsKeyMgmtExport(void* keyData, int selection, OSSL_CALLBACK* paramCb, void* cbArg)
{
    ...
    if (selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS)
    {
        ...
        ret &=
            OSSL_PARAM_BLD_push_utf8_string(tmpl, OSSL_PKEY_PARAM_GROUP_NAME, name, strlen(name));
    }

    if (selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY)
    {
        const BIGNUM* privateKey = GsAsymmKeyGet0PrivateKey(key);
        ret &= OSSL_PARAM_BLD_push_BN(tmpl, OSSL_PKEY_PARAM_PRIV_KEY, privateKey);
    }
    ...
    if (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY)
    {
        ...
        size_t publicKeyLen =
            EC_POINT_point2buf(group, publicKey, POINT_CONVERSION_COMPRESSED, &publicKeyBuf, bnCtx);
        ret &= OSSL_PARAM_BLD_push_octet_string(tmpl, OSSL_PKEY_PARAM_PUB_KEY, publicKeyBuf,
                                                publicKeyLen);
    }
