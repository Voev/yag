...
int GsKeyMgmtMatch(const void* keyDataA, const void* keyDataB, int selection)
{
    ...
    BN_CTX* ctx;
    int ret = 1;
    GsProvCtx* provCtx = GsAsymmKeyGet0ProvCtx(keyA);
...
static int GsKeyMgmtCheckPrivateKey(const GsAsymmKey* key)
{
    ...
    if (0 > BN_cmp(value, BN_value_one()) || 0 <= BN_cmp(value, EC_GROUP_get0_order(group)))
    {
        ErrRaise(GsAsymmKeyGet0ProvCtx(key), EC_R_INVALID_PRIVATE_KEY);
        return 0;
    }
    return 1;
}
...
static int GsKeyMgmtPairwiseCheck(const GsAsymmKey* key, BN_CTX* ctx)
{
    ...
    {
        ErrRaise(GsAsymmKeyGet0ProvCtx(key), ERR_R_EC_LIB);
        goto err;
    }
...
static int GsKeyMgmtCheckPublicKeyRange(const GsAsymmKey* key, BN_CTX* ctx)
{
    ...
    if (BN_is_negative(x) || BN_cmp(x, field) >= 0 || BN_is_negative(y) || BN_cmp(y, field) >= 0)
    {
        goto err;
    }
    ret = 1;
...
static int GsKeyMgmtCheckPublicKey(const GsAsymmKey* key, BN_CTX* ctx)
{
    ...
    {
        ErrRaise(provCtx, EC_R_POINT_AT_INFINITY);
        return 0;
    }
    ...
    {
        ErrRaise(provCtx, EC_R_COORDINATES_OUT_OF_RANGE);
        goto err;
    }

    if (0 >= EC_POINT_is_on_curve(group, pubValue, ctx))
    {
        ErrRaise(provCtx, EC_R_POINT_IS_NOT_ON_CURVE);
        goto err;
    }
    ...
    if (BN_is_zero(order))
    {
        ErrRaise(provCtx, EC_R_INVALID_GROUP_ORDER);
        goto err;
    }

    if (!EC_POINT_mul(group, point, NULL, pubValue, order, ctx))
    {
        ErrRaise(provCtx, ERR_R_EC_LIB);
        goto err;
    }
    if (!EC_POINT_is_at_infinity(group, point))
    {
        ErrRaise(provCtx, EC_R_WRONG_ORDER);
        goto err;
    }
    ret = 1;
err:
...
int GsKeyMgmtValidate(const void* keyData, int selection, int checktype)
{
    ...
    BN_CTX* ctx = BN_CTX_new_ex(GsProvCtxGet0LibCtx(provCtx));
    if (!ctx)
    {
        return 0;
    }
