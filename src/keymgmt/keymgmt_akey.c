...
GsAsymmKey* GsAsymmKeyNewInit(GsProvCtx* provCtx, int algorithm)
{
    GsAsymmKey* key = GsAsymmKeyNew();
    if (key)
    {
        GsAsymmKeySet0ProvCtx(key, provCtx);
...
void GsAsymmKeySet0ProvCtx(GsAsymmKey* key, GsProvCtx* ctx)
{
    OPENSSL_assert(key);
    key->provCtx = ctx;
}
...
int GsAsymmKeyDecodePublicKey(GsAsymmKey* key, const unsigned char* buf, size_t len)
{
    ...
    ctx = BN_CTX_new_ex(GsProvCtxGet0LibCtx(key->provCtx));
    if (!ctx)
    {
        goto end;
    }
...
GsProvCtx* GsAsymmKeyGet0ProvCtx(const GsAsymmKey* key)
{
    OPENSSL_assert(key);
    return key->provCtx;
}
