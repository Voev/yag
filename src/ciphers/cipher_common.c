...
void GsCipherCtxInit(GsCipherCtx* ctx, size_t keyBits, size_t blockBits, size_t ivBits,
                     unsigned int mode, ossl_unused uint64_t flags, const GsCipherSpec* spec,
                     void* provCtx)
{
    OPENSSL_assert(ctx);

    ctx->provCtx = (GsProvCtx*)ctx;
...
void* GsCipherDupCtx(void* ctx)
{
    ...
    if (NULL == ret)
    {
        ErrRaise(in->provCtx, ERR_R_MALLOC_FAILURE);
        return NULL;
    }
...
static int CipherInit(void* vctx, const unsigned char* key, size_t keyLength,
                      const unsigned char* iv, size_t ivLenght, const OSSL_PARAM params[], int enc)
{
    ...
    if (key != NULL)
    {
        ...
        {
            ErrRaise(ctx->provCtx, PROV_R_INVALID_KEY_LENGTH);
            return 0;
        }
...
int GsCipherEncryptInit(void* vctx, const unsigned char* key, size_t keyLength,
                        const unsigned char* iv, size_t ivLenght, const OSSL_PARAM params[])
{
...
int GsCipherCtxSetIv(GsCipherCtx* ctx, const unsigned char* iv, size_t ivLength)
{
    ...
    {
        ErrRaise(ctx->provCtx, PROV_R_INVALID_IV_LENGTH);
        return 0;
    }
    ctx->ivSetted = 1;
...
size_t FillBlock(unsigned char* buf, size_t* buflen, size_t blocksize, const unsigned char** in,
                 size_t* inlen)
{
...
int TrailData(GsProvCtx* ctx, unsigned char* buf, size_t* buflen, size_t blocksize,
              const unsigned char** in, size_t* inlen)
{
    ...
    {
        ErrRaise(ctx, ERR_R_INTERNAL_ERROR);
        return 0;
    }
                        ...
                        size_t outsize, const unsigned char* in, size_t inl)
{
    ...
    if (ctx->bufferSize == blksz && (ctx->enc || inl > 0 || !ctx->pad))
    {
        if (outsize < blksz)
        {
            ErrRaise(ctx->provCtx, PROV_R_OUTPUT_BUFFER_TOO_SMALL);
            return 0;
        }
        if (!ctx->spec->cipher(ctx, out, ctx->buffer, blksz))
        {
            ErrRaise(ctx->provCtx, PROV_R_CIPHER_OPERATION_FAILED);
            return 0;
        }
        ctx->bufferSize = 0;
        outlint = blksz;
...
int MakeBlockUnpadding(GsProvCtx* provCtx, unsigned char* buf, size_t* buflen, size_t blocksize)
{
    size_t pad, i;
    size_t len = *buflen;
    ...
    if (len != blocksize)
    {
        ErrRaise(provCtx, ERR_R_INTERNAL_ERROR);
        return 0;
    }
    ...
    {
        ErrRaise(provCtx, PROV_R_BAD_DECRYPT);
        return 0;
    }
    for (i = 0; i < pad; i++)
    {
...
int GsCipherBlockFinal(void* vctx, unsigned char* out, size_t* outl, size_t outsize)
{
    GsCipherCtx* ctx = INTERPRET_AS_CIPHER_CTX(vctx);
    size_t blksz = ctx->blockSize;
    ...
    if (ctx->enc)
    {
        ...
        {
            ErrRaise(ctx->provCtx, PROV_R_WRONG_FINAL_BLOCK_LENGTH);
            return 0;
        }
...
int GsCipherCipher(void* vctx, unsigned char* out, size_t* outLength, size_t outSize,
...
int GsCipherGetParams(OSSL_PARAM params[], unsigned int mode, ossl_unused uint64_t flags,
                      size_t keyBits, size_t blockBits, size_t ivBits)
{
    OSSL_PARAM* p;
...
int GsCipherSetCtxParams(void* vctx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL)
    {
        ...
        {
            ErrRaise(ctx->provCtx, PROV_R_FAILED_TO_GET_PARAMETER);
            return 0;
        }
        ctx->pad = pad ? 1 : 0;
    }
